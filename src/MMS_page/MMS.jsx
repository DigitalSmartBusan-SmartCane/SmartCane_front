...
function MMSPage() {
  ...
  const fetchRegistrations = async () => {
    try {
      const response = await fetch("http://127.0.0.1:8080/api/register/registrations");
      ;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8080/api/register/registrations/${id}`, {
        method: "DELETE",
      });
  ...
  const handleRelationChange = async (index, value) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8080/api/register/registrations/relation", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
